...
primsData = read.csv('~/Learning/Data/Algo2/Ass01/edges.txt', sep = "", header = F)
...
all_vertices = 1:500
in_vertices = data.frame(start = 100, acc_weight = 0)
...
for(i in 1:500)
{
  ...
  current_boundary_weights$total = current_boundary_weights$weight 
  ...
  graphData_full = subset(graphData_full, !(graphData_full$end %in% in_vertices$start))
  
}

sum(in_vertices$acc_weight)
