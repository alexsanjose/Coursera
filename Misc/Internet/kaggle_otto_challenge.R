...
rm(list=ls())
# Load data
train <- read.csv("../Data/OttoChallenge/train.csv")
test <- read.csv("../Data/OttoChallenge/test.csv")
samplesub <- read.csv("../Data/OttoChallenge/sampleSubmission.csv")
...
# Determine the error rate for the model
fit3$error <- 1-(sum(fit3.pred$class==stest$target)/length(stest$target))
fit3$error
